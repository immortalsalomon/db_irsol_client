...
class MeasurementParser:
    ...
    def __post_parse_measurement_parameters(measurement_file_path, parameters, fields_to_columns):
        ...
        if 'DATE' in fields_to_columns and fields_to_columns['DATE'] in parameters:
            parameters[fields_to_columns['DATE']] = re.sub("\\+.*|\\}|\\{|T", " ",
                                                           parameters[fields_to_columns['DATE']]).strip()

        if 'DATE_END' in fields_to_columns and fields_to_columns['DATE_END'] in parameters:
            parameters[fields_to_columns['DATE_END']] = re.sub("\\+.*|\\}|\\{|T", " ",
                                                               parameters[fields_to_columns['DATE_END']]).strip()

        if fields_to_columns['WLENGTH'] not in parameters or parameters[fields_to_columns['WLENGTH']] is None \
                or parameters[fields_to_columns['WLENGTH']] == 0:
